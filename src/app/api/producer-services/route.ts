...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { producerId, title, bpm, genre, price, link }: ProducerBeatData = await req.json();

    if (producerId !== decoded.uid) {
      return new NextResponse(JSON.stringify({ error: 'Unauthorized producerId' }), { status: 403 });
    }
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return new NextResponse(JSON.stringify({ beats }), { status: 200 });
  } catch (err) {
    return new NextResponse(JSON.stringify({ error: (err as Error).message }), { status: 500 });
  }
}
